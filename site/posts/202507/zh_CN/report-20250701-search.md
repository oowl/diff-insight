...
summary: |-
  此次代码更新主要是在 Azure AI Search 文档中增加了关于 JavaScript 和 TypeScript 的向量搜索快速入门指南，并更新了相关的概述文档链接。这一变更为用户提供了针对不同编程语言执行向量搜索的支持与便利性。

  新功能方面，新增了两个快速入门文档，分别提供了 JavaScript 和 TypeScript 的向量搜索示例与实现指南。此外，没有识别到破坏性更改，更新属于增量性质。同时，概述文档中的链接也进行了相应的更新。

  总体来看，此次更新为开发者提供了更多语言选择和实例支持，帮助他们更快速地掌握向量搜索的实现，提升了用户体验，并促进了 Azure AI Search 在开发社区中的应用。
...
这段代码更新主要涉及在 Azure AI Search 文档中，新增了一个关于 JavaScript 和 TypeScript 的向量搜索快速入门指南。同时，还在相关的概述文档中更新了链接引导。这些修改增加了用户针对不同编程语言执行向量搜索的指导和便利性。

## New features

- 增加了“快速入门：获取开始使用向量搜索的 JavaScript 示例”文档，提供了完整的向量搜索指南及代码示例。
- 增加了“快速入门：获取开始使用向量搜索的 TypeScript 示例”文档，提供了详细的 TypeScript 向量搜索实现步骤。
...
- 更新了“向量搜索指南”概述文档，加入了 JavaScript 和 TypeScript 两个快速入门指南的链接。

# Insights

在此次文档更新中，主要的改进是在向量搜索的学习路径上给予开发者更多的语言选择和示例支持。通过新增 JavaScript 和 TypeScript 的快速入门指南，Azure AI Search 的功能示例更加全面。这样的文档改进不仅详细涵盖了技术实现的步骤和代码示例，而且引入了向量存储、索引架构以及 API 调用的基本概念，使得开发者可以更快速上手。

对于习惯于使用不同编程语言的开发者，这种快速入门的形式是非常有帮助的，可以帮助他们迅速搭建起开发环境并进行基本操作。同时，示例中提到的高级搜索功能，如带过滤的搜索、地理空间搜索等，为用户展示了如何利用更复杂的搜索技术来满足特定的业务需求。

将这些内容整合到主文档的代价不高，但它的附加值很大。它给予开发者加速学习和项目实施的机会，并促进了 Azure 在现代化搜索需求中的适应和应用。最终，这类文档的优化将提升用户体验，并增加 Azure AI Search 在社区中的采用率。

# Summary Table
|  Filename  | Type |    Title    | Status | A  | D  | M  |
|------------|------|-------------|--------|----|----|----|
| [search-get-started-vector-javascript.md](#item-d0387c) | new feature | 快速入门：获取开始使用向量搜索的 JavaScript 示例 | added | 518 | 0 | 518 | 
| [search-get-started-vector-typescript.md](#item-9b3bc8) | new feature | 快速入门：获取开始使用向量搜索的 TypeScript 示例 | added | 555 | 0 | 555 | 
| [search-get-started-vector.md](#item-4984d9) | minor update | 更新向量搜索指南，添加 JavaScript 和 TypeScript 快速入门链接 | modified | 12 | 0 | 12 | 
...
```json
{
    ...
    "modification_title": "快速入门：获取开始使用向量搜索的 JavaScript 示例"
}
```

### Explanation
此次修改在 `articles/search/includes/quickstarts/search-get-started-vector-javascript.md` 文件中新增了一个完整的快速入门指南，该指南展示了如何使用 JavaScript 来创建、加载和查询向量。该文档提供了详细的代码示例和步骤说明，涵盖了如何利用 Azure AI Search 客户端库进行数据的索引操作、加载文档、构建查询等基本操作。

新增的内容包括：支持向量存储的索引架构、向量化步骤的省略说明、项目设置的先决条件、环境变量的配置，以及逐步指导如何通过 Node.js 项目与 Azure AI Search 及 Azure OpenAI 进行连接。此外，该文档还示范了如何执行单一向量搜索、带过滤的新搜索、地理空间筛选及混合搜索等高级功能。整体而言，该功能的增加显著增强了用户在 Azure 中使用向量搜索的便利性和可操作性。
...
{
    "modification_type": "new feature",
    "modification_title": "快速入门：获取开始使用向量搜索的 TypeScript 示例"
}
```

### Explanation
此次修改在 `articles/search/includes/quickstarts/search-get-started-vector-typescript.md` 文件中新增了一个完整的快速入门指南，该指南展示了如何使用 TypeScript 来创建、加载和查询向量。文档通过 Azure AI Search 客户端库提供了详细的代码示例，涵盖了索引操作、数据加载以及查询的基本步骤。

新增内容包括：定义向量存储的索引架构、向量搜索配置、项目设置的先决条件、如何配置环境变量，以及通过 Node.js 与 Azure AI Search 和 Azure OpenAI 进行 API 调用的详细步骤。此外，文档还包含了对单一向量搜索、带过滤的搜索、地理空间过滤和混合搜索等高级功能的实现示范。这次更新使得用户在 Azure 上使用 TypeScript 进行向量搜索变得更加容易和直观。
...
```json
{
    ...
    "modification_title": "更新向量搜索指南，添加 JavaScript 和 TypeScript 快速入门链接"
}
```

### Explanation
此次修改在 `articles/search/search-get-started-vector.md` 文件中增加了对 JavaScript 和 TypeScript 的快速入门指南的引用。在原有内容的基础上，新增部分包括了两个新的区域展示，它们分别针对 JavaScript 和 TypeScript 语言。

更新的内容通过 `::: zone pivot="javascript"` 和 `::: zone pivot="typescript"` 的语法来引入相应的快速入门教程链接，使得用户可以更方便地访问到对应的资源。这个小改动旨在提高文档的可用性，为使用不同编程语言的开发者提供更加直接的学习路径，从而帮助他们更好地理解如何实施向量搜索功能。
